...
class DiffAE(nn.Model):
    def __init__(self, conf, vocab_size, PAD_token=0):
        ...
        self.context_encoder = ContextEncoder(
                                    ...
                                    conf['n_hidden'], 1, 
                                    conf['noise_radius']) 
        self.prior_net = LatentEncoder(conf['n_hidden'], conf['z_size']) # latent encoder B
        self.post_net = LatentEncoder(conf['n_hidden']*3, conf['z_size']) # latent encoder A
        ######### TODO #######
        ...
        self.lr_scheduler_AE_phase1 = optim.lr_scheduler.StepLR(
                                            self.optimizer_AE_phase1, 
                                            step_size = 10, 
                                            gamma=0.6)
        self.lr_scheduler_AE_phase2 = optim.lr_scheduler.StepLR(
                                            self.optimizer_AE_phase2, 
                                            step_size = 10, 
                                            gamma=0.6)

    def get_diff_data(self, context, response, context_lens, utt_lens, floors, res_lens):
        print("Sampling latent encoder A...")
        self.post_net.eval()
        with torch.no_grad():
    ...
    def sample_diffuser(self, num_samples, ):
        ...
        self.diffuser.eval()
        sample = torch.randn(num_samples, self.z_size)
    ...
    def train_AE_phase1(self, context, response, context_lens, utt_lens, floors, res_lens):
        ...
        for _ in enumerate(tqdm(range(self.decoder_phase1_train_epoch))):
            ...
            nn.utils.clip_grad_norm_(
                list(self.context_encoder.parameters())+list(self.post_net.parameters()), 
                self.clip)
    ...
    def train_AE_phase2(self, context, response, context_lens, utt_lens, floors, res_lens):
        ...
        for _ in enumerate(tqdm(range(self.decoder_phase1_train_epoch))):
            ...
            loss.backward()
            nn.utils.clip_grad_norm_(
                list(self.context_encoder.parameters())+list(self.prior_net.parameters())+
                list(self.decoder.parameters())+list(self.respsonse_decoder.parameters()), 
                self.clip)
    ...
    def adjust_lr(self):
        self.self.lr_scheduler_AE_phase1.step()
        self.self.lr_scheduler_AE_phase2.step()

    def valid(self, context, response, context_lens, utt_lens, floors, res_lens):
